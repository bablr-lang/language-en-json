...
export default class JSON {
  ...
  *Object() {
    ...
    while (sep && (yield match(m`/[^}]/s`))) {
      yield match(m`<__All />`, freezeRecord([m`key$: <//>`, m`sigilToken*: <* ':' />`]));
  ...
  *Number() {
    yield eatMatch(m`sign*: <* /[+-]/ />`, o({}), o({ bind: true }));

    yield eat(m`wholePart$: <*UnsignedInteger />`, o({ noDoubleZero: true }));

    let ds = yield eatMatch(m`decimalSeparator*: <* '.' />`, o({}), o({ bind: true }));

    if (ds) {
      yield eat(m`decimalPart$: <*UnsignedInteger />`);
    }

    let es = yield eatMatch(m`exponentSeparator*: <* /[eE]/ />`, o({}), o({ bind: true }));

    yield eatMatch(m`exponentSign*: <* /[+-]/ />`);

    if (es) {
      yield eat(m`exponentPart$: <*UnsignedInteger />`);
    }
  }
