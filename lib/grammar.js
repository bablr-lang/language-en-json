 ...
 * @generatedfrom ./grammar.macro.js#c3203ffc68937520509fc0e9848ce60fba5c3f1a
...
export const grammar = basicTriviaEnhancer({
  ...
  *EscapeCode() {
    if (yield eatMatch(m`typeToken: <*Keyword 'u' />`)) {
      if (yield eatMatch(m`openToken: <* '{' { balanced: '}' } />`, null, o({
        bind: true
      }))) {
  ...
  *Number() {
    ...
    let fs = yield eatMatch(m`fractionalSeparatorToken: <* '.' />`, null, o({
      bind: true
    }));
    if (fs) {
    ...
    let es = yield eatMatch(m`exponentSeparatorToken: <* /[eE]/ />`, null, o({
      bind: true
    }));
    if (es) {
  ...
  *Integer({
    ...
    if (matchSign) {
      yield eatMatch(m`signToken: <* ${buildString(matchSign)} />`, null, o({
        bind: true
      }));
    } else {
